...
pub fn select_task_with_fzf(tasks: &Vec<String>, args: &Vec<String>) -> Result<Option<Option<String>>, std::io::Error> {
    ...
    child.stdin.as_mut().ok_or_else(|| std::io::Error::new(std::io::ErrorKind::Other, "Failed to open stdin"))?
        .write_all(tasks.as_bytes())?;

    let output = child.wait_with_output()?;

    let selected_task = String::from_utf8(output.stdout).map_err(|_| std::io::Error::new(std::io::ErrorKind::Other, "Failed to convert output to UTF-8"))?.trim().to_string();
